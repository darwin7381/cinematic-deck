          ...
          <>
            ...
            <div className={`relative z-10 h-full flex flex-col ${isPortrait ? 'px-[60px] py-[120px]' : 'px-[140px] py-[90px]'}`}>
              <motion.div
                ...
                className={`font-mono tracking-[0.4em] text-cyan-300/70 uppercase ${isPortrait ? 'text-base mb-8' : 'text-xs mb-6'}`}
              >
              ...
              <motion.h2
                ...
                className={`font-bold leading-[1] tracking-[-0.03em] text-white ${isPortrait ? 'text-[72px] mb-4' : 'text-[80px] mb-4'}`}
              >
                React vs Vue, when
                <br />
              ...
              <motion.p
                ...
                className={`text-white/45 font-light ${isPortrait ? 'text-base mb-8' : 'text-lg mb-10'}`}
              >
                The gap isn't 2× or 3×. It's an order of magnitude.
              </motion.p>

              {/* Portrait: stack rows vertically with smaller grid · Landscape: full 3-column grid */}
              {isPortrait ? (
                <div className="flex-1 flex flex-col gap-[14px]">
                  {ROWS.map((row, i) => (
                    ...
                    >
                      <div className="text-white/70 text-[20px] font-medium mb-1">{row.tool}</div>
                      {/* React row */}
                      <div className="flex items-center gap-3">
                        <span className="text-cyan-300/80 font-mono text-[11px] w-10">REACT</span>
                        ...
                        <span className="font-mono text-sm text-cyan-300/80 w-[52px] text-right">{formatStars(row.reactNum)}⭐</span>
                      </div>
                      {/* Vue row */}
                      <div className="flex items-center gap-3">
                        <span className="text-emerald-300/60 font-mono text-[11px] w-10">VUE</span>
                        <div className="flex-1 h-7 relative bg-white/5 rounded-md overflow-hidden">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: row.vueNum > 0 ? `${(row.vueNum / maxStars) * 100}%` : '0%' }}
                            transition={{ duration: 1, delay: 0.75 + i * 0.08, ease: [0.22, 1, 0.36, 1] }}
                            className={row.vueNum > 0 ? 'h-full bg-gradient-to-r from-emerald-500/40 to-teal-500/40' : ''}
                          />
                          <div className="absolute inset-0 flex items-center px-3 text-[13px] font-mono text-white/60">
                            {row.vue}
                          </div>
                        </div>
                        <span className="font-mono text-sm text-emerald-300/70 w-[52px] text-right">
                          {formatStars(row.vueNum)}{row.vueNum > 0 ? '⭐' : ''}
                        </span>
                      </div>
                    </motion.div>
                  ))}
                  ...
                  {ROWS.map((row, i) => (
                    <div key={row.tool} className="contents">
                      ...
                      >
                        ...
                        <span className="font-mono text-sm text-emerald-300/70 w-[44px] text-right">
                          {formatStars(row.vueNum)}{row.vueNum > 0 ? '⭐' : ''}
                        </span>
                      </motion.div>
                    </div>
              ...
              <motion.div
                ...
                className={`font-mono text-white/50 border-l-2 border-cyan-400/40 pl-5 ${isPortrait ? 'mt-6 text-base max-w-full' : 'mt-10 text-sm max-w-[900px]'}`}
              >
